...
<body class="sb-nav-fixed">
    ...
    <a class="navbar-brand ps-3" href="index.html">Home</a>
        ...
        <div id="layoutSidenav_content" style="margin-left: 250px; padding: 20px;">
            <main>
                <h2 style="margin: 25px;">Listado de Clientes</h2>
            ...
            <table class="tabla-clientes" style="padding: 20px; border-collapse: separate; border-spacing: 20px 0;">
                <thead class ="bg-gray-50">
                    <tr>
                        ...
                        <th style="padding-right: 30px" scope="col-">Contrasena</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class ="text-center"><?= $datos->idcliente?></td>
                        <td><?= $datos->nombre ?? $datos->apellido ?? '' ?></td>
                        <td><?= $datos->apellido ?? $datos->apellido ?? '' ?></td>
                        <td><?= $datos->email ?? $datos->email ?? '' ?></td>
                        <td><?= $datos->contrasena ?? $datos->contrasena ?? '' ?></td>

                        <td>
                            <form class="Baja-Clientes-Form" id="Baja-Clientes-Form" action="../Admin-Dashboard/ABML-Clientes/Baja-Clientes.php" method="POST" style="display:inline;">
                                <input type="hidden" name="idcita" value="<?= $datos->idcliente ?>" />
                                <button type="submit" class="btn btn-small btn-warning" onclick="return confirm('¿Está seguro de eliminar este cliente?');">
